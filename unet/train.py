            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.n_classes == 1:
                        ...
                        loss += dice_l
                        #print(f'dice_loss= {dice_l}')
                        if global_step % 500== 0:
                            viz_img = true_masks[0]
                            viz_pred = masks_pred[0].argmax(dim=0)
                            plot_img_and_mask(viz_img.cpu().numpy(), viz_pred.cpu().numpy(), f'./img_train/{cnt}')
                            cnt += 1
                ...
                # Evaluation round
                division_step = (train_size // (5 * batch_size))
                if division_step > 0:
